package net.librebounce.features.module.impl.world.scaffold
...
open class EagleComponent(owner: Module, generalApply: () -> Boolean = { true }): Configurable("ClickingSettings"),
    MinecraftInstance, Listenable {
    private val eagle by
        choices("Eagle", arrayOf("Normal", "Silent", "Off"), "Normal") { generalApply() }
    private val eagleMode by choices("EagleMode", arrayOf("Both", "OnGround", "InAir"), "Both")
    { generalApply() && eagle != "Off" }
    private val adjustedSneakSpeed by boolean("AdjustedSneakSpeed", true)
    { generalApply() && eagle == "Silent"}
    private val eagleSpeed by float("EagleSpeed", 0.3f, 0.3f..1.0f)
    { generalApply() && eagle != "Off" }
    val eagleSprint by boolean("EagleSprint", false) { generalApply() && eagle == "Normal" }
    ...
    { generalApply() && eagle != "Off" }
    private val useMaxSneakTime by boolean("UseMaxSneakTime", true) { generalApply() && eagle != "Off" }
    private val maxSneakTicks by intRange("MaxSneakTicks", 3..3, 0..10)
    { generalApply() && eagle != "Off" && useMaxSneakTime }
    ...
    private val debug by boolean("Debug", false) { generalApply() }
